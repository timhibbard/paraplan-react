...
export default class App extends Component {
  constructor(props) {
    ...
    this.initialState = {
      ...
      tripEndDate: new Date(new Date().setHours(48, 0, 0, 0)).getTime() / 1000,
      routes: [],
      dispatcherTrips: [],
      tripRequests: [],
      config: {},
      ...
      .then(response => {
        ...
        this.setState({
          ...
          dispatcherTrips: this.state.dispatcherTrips.map(el =>
            el.tripId === tripId ? { ...el, fleetmanager } : el
          )
        })
      })
      .catch(reason => {
      ...
      .then(response => {
        ...
        this.setState({
          ...
          tripRequests: this.state.tripRequests.map(el =>
            el.importTripID === importTripID ? { ...el, tripStatus } : el
          )
        })
      })
      .catch(reason => {
  ...
  addTripToAPI() {
    ...
    var request = {
      ...
      trip: {
        ...
        scheduledPickUpTime: moment()
          .add(5, 'minutes')
          .unix(),
        scheduledDropOffTime: moment()
          .add(30, 'minutes')
          .unix(),
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <ul>
          {allVehicles.map((v, i) => {
            return <li key={v.databaseID}>{v.year + ' ' + v.make}</li>
          })}
          {allDrivers.map((d, i) => {
            return <li key={d.databaseID}>{d.firstName + ' ' + d.lastName}</li>
          })}
          {allRoutes.map((r, i) => {
            return <li key={r.databaseID}>{r.name}</li>
          })}
          {wheelchairTypes.map((wc, i) => {
            return <li key={i}>{wc.wheelchairName}</li>
          })}
          ...
          {routes.map((route, i) => {
            return (
              <li key={route.fleetmanagerID}>
                {route.routeName}
                {route.gps
                  ? ' GPS: ' + route.gps.lat + ', ' + route.gps.lng
                  : ''}
              </li>
            )
          })}
              ...
              <li key={trip.tripId}>
                ...
                {routes.map((route, i) => {
                  return (
                    ...
                    >
                      Assign to {route.routeName} ({route.fleetmanagerID})
                    </button>
                  )
                })}
              </li>
            ...
            ? tripRequests.map((trip, i) => {
                return (
                  <li key={trip.importTripID}>
                    ...
                    {moment.unix(trip.pickUpTimeEpoch).format('ll')}
                    <button
                      style={buttonStyle}
